...
- Every document must have an `_id` field. Use distributed ID (generated by `LeafId.next()`) as the primary key strategy.
- Store ID as `Long` (int64) type in `_id`. Do not use MongoDB auto-generated `ObjectId`.
  - OK: `{ _id: NumberLong(1867452390812348416) }` (distributed ID as int64)
...
- Distributed ID (Long) advantages over ObjectId:
...
- Why Long over String for distributed ID:
  ...
  - Consistency: Distributed ID is inherently a 64-bit integer; storing as String is a type mismatch
- **CAUTION**: Distributed IDs exceed JavaScript `Number.MAX_SAFE_INTEGER` (2^53-1). When exposing IDs via REST API, serialize as String in JSON to prevent precision loss on JS clients. Use Jackson global Long-to-String serialization (see `references/spring-data-rules.md` Section 9).
...
- Use `Long` (int64) type for distributed ID identifiers, not `ObjectId` or `String`.
...
// BAD: Using String for distributed ID (wastes storage and index memory)
...
```javascript
// OK: Distributed ID as Long (time-sortable, distributed, compact)
