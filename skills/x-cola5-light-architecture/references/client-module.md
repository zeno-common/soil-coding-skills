...
> 本文档描述的 `client` 是**顶层 Maven 模块**（与 `app` 实现模块同级），而非 `adapter` 内的子包。其坐标可独立 `deploy` 到 Maven 仓库，供消费方仅以 jar 依赖。

对外契约包（external contract package）：服务间调用的 API 契约，**独立发布为 jar**，供消费方以 Maven 依赖引入。**独立于四层之外，不依赖任何业务模块。**
...
> 本规范中 client 为独立契约包，应 `deploy` 到 Maven 仓库；消费方仅依赖 `client` jar，不依赖 `adapter`。
